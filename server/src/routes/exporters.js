...
router.get("/", requireAuth, requireRole("ADMIN"), async (req, res) => {
  ...
  const params = [];
  let sql = `SELECT id, nombre, estado, created_at FROM exporters`;
...
router.put("/:id", requireAuth, requireRole("ADMIN"), async (req, res) => {
  ...
  await pool.query(
    `INSERT INTO exporters (id, nombre, estado)
     VALUES ($1,$2,$3)
...
router.patch("/:id/estado", requireAuth, requireRole("ADMIN"), async (req, res) => {
  ...
  if (!["ACTIVO","INACTIVO"].includes(estado)) return res.status(400).json({ error: "estado inválido" });
  const r = await pool.query(`UPDATE exporters SET estado=$1 WHERE id=$2`, [estado, id]);
  if (!r.rowCount) return res.status(404).json({ error: "No existe el exportador" });
  res.json({ ok: true });
});
